...
function VerifyEmailInner() {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        const res = await api.post("/organisers/auth/verify-email", { token });
  ...
  const onResend = async (values: ResendValues) => {
    ...
    try {
      const res = await api.post("/organisers/auth/resend-verification", { email: values.email });
      const payload = res.data;
      if (payload?.isSuccess === false) {
        ...
        <div className="w-full max-w-md">
          <Card className="shadow-lg border-black/10 bg-white">
            <CardHeader className="text-center">
              ...
              <p className="text-sm text-black/70 mt-2">For your GoDo organiser account.</p>
            </CardHeader>
            ...
            <CardContent>
              ...
              {status === "success" && (
                <div className="space-y-4 text-center">
                  ...
                  <p className="text-sm text-black/70">
                    You can now log in and start publishing your events.
                  </p>
                </div>
              )}
              ...
              {(status === "failed" || status === "missing-token") && (
